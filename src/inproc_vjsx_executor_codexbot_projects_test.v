...
fn test_inproc_vjsx_executor_repo_codexbot_app_ts_projects_and_models_commands_use_selection_scope() {
	codexbot_ts_with_temp_db('codexbot_ts_projects_models.sqlite', fn (_ string) {
		...
		setting_resp := codexbot_ts_dispatch_feishu_message(mut executor, mut app,
			'codexbot_ts_projects_models_setting', 'trace_codexbot_ts_projects_models_setting',
			'chat_codexbot_ts_projects_models', 'om_codexbot_ts_projects_models_setting',

			'/setting project_root_dir ' + project_root) or { panic(err) }
		...
		create_alpha := codexbot_ts_dispatch_feishu_message(mut executor, mut app,
			'codexbot_ts_projects_models_create_alpha',
			'trace_codexbot_ts_projects_models_create_alpha', 'chat_codexbot_ts_projects_models',
			'om_codexbot_ts_projects_models_create_alpha', '/create alpha') or { panic(err) }
		...
		bind_beta := codexbot_ts_dispatch_feishu_message(mut executor, mut app,
			'codexbot_ts_projects_models_bind_beta', 'trace_codexbot_ts_projects_models_bind_beta',
			'chat_codexbot_ts_projects_models', 'om_codexbot_ts_projects_models_bind_beta',
			'/bind beta') or { panic(err) }
		assert bind_beta.handled
		assert bind_beta.commands.len == 1
		assert bind_beta.commands[0].text.contains('Project: `beta`')

		alpha_task, alpha_stream_id := codexbot_ts_start_task(mut executor, mut app,
			'codexbot_ts_alpha_task', 'trace_codexbot_ts_alpha_task',
			'chat_codexbot_ts_projects_models', 'om_codexbot_ts_alpha_task', 'alpha task') or {
			panic(err)
		}
		assert alpha_task.handled
		assert alpha_stream_id != ''
		_ = codexbot_ts_dispatch_codex_event(mut executor, mut app, 'codexbot_ts_alpha_rpc',
			alpha_stream_id, 'codex.rpc.response',
			'{"method":"thread/start","result":{"threadId":"thread_project_alpha"},"has_error":false}') or {
			panic(err)
		}
		_ = codexbot_ts_dispatch_codex_event(mut executor, mut app, 'codexbot_ts_alpha_done',
			alpha_stream_id, 'codex.notification', '{"method":"turn/completed","params":{}}') or {
			panic(err)
		}
		_ = codexbot_ts_dispatch_codex_event(mut executor, mut app, 'codexbot_ts_alpha_read',
			alpha_stream_id, 'codex.rpc.response',
			'{"method":"thread/read","result":{"thread":{"id":"thread_project_alpha","turns":[{"id":"turn_thread_project_alpha","items":[{"type":"agentMessage","id":"item_thread_project_alpha","text":"alpha result","phase":"final_answer","memoryCitation":null}],"status":"completed","error":null}]}},"has_error":false}') or {
			panic(err)
		}

		switch_beta := codexbot_ts_dispatch_feishu_message(mut executor, mut app,
			'codexbot_ts_switch_beta', 'trace_codexbot_ts_switch_beta',
			'chat_codexbot_ts_projects_models', 'om_codexbot_ts_switch_beta', '/project beta') or {
			panic(err)
		}
		assert switch_beta.commands[0].text.contains('Project: `beta`')

		beta_task, beta_stream_id := codexbot_ts_start_task(mut executor, mut app,
			'codexbot_ts_beta_task', 'trace_codexbot_ts_beta_task',
			'chat_codexbot_ts_projects_models', 'om_codexbot_ts_beta_task', 'beta task') or {
			panic(err)
		}
		assert beta_task.handled
		assert beta_stream_id != ''
		_ = codexbot_ts_dispatch_codex_event(mut executor, mut app, 'codexbot_ts_beta_rpc',
			beta_stream_id, 'codex.rpc.response',
			'{"method":"thread/start","result":{"threadId":"thread_project_beta"},"has_error":false}') or {
			panic(err)
		}
		_ = codexbot_ts_dispatch_codex_event(mut executor, mut app, 'codexbot_ts_beta_done',
			beta_stream_id, 'codex.notification', '{"method":"turn/completed","params":{}}') or {
			panic(err)
		}
		_ = codexbot_ts_dispatch_codex_event(mut executor, mut app, 'codexbot_ts_beta_read',
			beta_stream_id, 'codex.rpc.response',
			'{"method":"thread/read","result":{"thread":{"id":"thread_project_beta","turns":[{"id":"turn_thread_project_beta","items":[{"type":"agentMessage","id":"item_thread_project_beta","text":"beta result","phase":"final_answer","memoryCitation":null}],"status":"completed","error":null}]}},"has_error":false}') or {
			panic(err)
		}

		projects_resp := codexbot_ts_dispatch_feishu_message(mut executor, mut app,
			'codexbot_ts_projects_list', 'trace_codexbot_ts_projects_list',
			'chat_codexbot_ts_projects_models', 'om_codexbot_ts_projects_list', '/projects') or {
			panic(err)
		}
		...
		use_project_resp := codexbot_ts_dispatch_feishu_message(mut executor, mut app,
			'codexbot_ts_projects_use_alpha', 'trace_codexbot_ts_projects_use_alpha',
			'chat_codexbot_ts_projects_models', 'om_codexbot_ts_projects_use_alpha', '/use alpha') or {
			panic(err)
		}
		...
		use_project_resp_2 := codexbot_ts_dispatch_feishu_message(mut executor, mut app,
			'codexbot_ts_projects_use_beta_again', 'trace_codexbot_ts_projects_use_beta_again',
			'chat_codexbot_ts_projects_models', 'om_codexbot_ts_projects_use_beta_again',
			'/use beta') or { panic(err) }
		...
		models_resp := codexbot_ts_dispatch_feishu_message(mut executor, mut app,
			'codexbot_ts_models_list', 'trace_codexbot_ts_models_list',
			'chat_codexbot_ts_projects_models', 'om_codexbot_ts_models_list', '/models') or {
			panic(err)
		}
		...
		use_model_resp := executor.dispatch_websocket_upstream(mut app, transport.WorkerWebSocketUpstreamDispatchRequest{
			...
			target_type: 'chat_id'
			payload:     codexbot_ts_feishu_payload('/use gpt-5.3-codex',
				'chat_codexbot_ts_projects_models', 'om_codexbot_ts_models_use')
		}) or { panic(err) }
		...
		use_model_resp_2 := executor.dispatch_websocket_upstream(mut app, transport.WorkerWebSocketUpstreamDispatchRequest{
			...
			target:      'chat_codexbot_ts_projects_models'
			target_type: 'chat_id'
			payload:     codexbot_ts_feishu_payload('/use gpt-5.4',
				'chat_codexbot_ts_projects_models', 'om_codexbot_ts_models_use_default_again')
		}) or { panic(err) }
		...
		use_after_scope_cleared := executor.dispatch_websocket_upstream(mut app, transport.WorkerWebSocketUpstreamDispatchRequest{
			...
			target:      'chat_codexbot_ts_projects_models'
			target_type: 'chat_id'
			payload:     codexbot_ts_feishu_payload('/use alpha',
		...
		use_after_scope_cleared_stream_id :=
			codexbot_ts_first_stream_id(use_after_scope_cleared.commands)
...
fn test_inproc_vjsx_executor_repo_codexbot_app_ts_new_command_can_switch_model_and_clear_thread() {
	codexbot_ts_with_temp_db('codexbot_ts_new_model.sqlite', fn (_ string) {
		...
		task_resp, stream_id := codexbot_ts_start_task(mut executor, mut app,
			'codexbot_ts_new_model_task', 'trace_codexbot_ts_new_model_task',
			'chat_codexbot_ts_new_model', 'om_codexbot_ts_new_model_task', 'seed thread') or {
			panic(err)
		}
		assert task_resp.handled
		assert stream_id != ''

		_ = codexbot_ts_dispatch_codex_event(mut executor, mut app, 'codexbot_ts_new_model_rpc',
			stream_id, 'codex.rpc.response',
			'{"method":"thread/start","result":{"threadId":"thread_new_model_001"},"has_error":false}') or {
			panic(err)
		}

		reset_resp := codexbot_ts_dispatch_feishu_message(mut executor, mut app,
			'codexbot_ts_new_model_reset', 'trace_codexbot_ts_new_model_reset',
			'chat_codexbot_ts_new_model', 'om_codexbot_ts_new_model_reset', '/new gpt-5.3-codex') or {
			panic(err)
		}
		...
		thread_resp := codexbot_ts_dispatch_feishu_message(mut executor, mut app,
			'codexbot_ts_new_model_thread', 'trace_codexbot_ts_new_model_thread',
			'chat_codexbot_ts_new_model', 'om_codexbot_ts_new_model_thread', '/thread') or {
			panic(err)
		}
		...
		model_resp := codexbot_ts_dispatch_feishu_message(mut executor, mut app,
			'codexbot_ts_new_model_model', 'trace_codexbot_ts_new_model_model',
			'chat_codexbot_ts_new_model', 'om_codexbot_ts_new_model_model', '/model') or {
			panic(err)
		}
...
fn test_inproc_vjsx_executor_repo_codexbot_app_ts_create_import_and_bind_projects() {
	codexbot_ts_with_temp_db('codexbot_ts_project_registry.sqlite', fn (_ string) {
		...
		setting_resp := codexbot_ts_dispatch_feishu_message(mut executor, mut app,
			'codexbot_ts_setting_project_root', 'trace_codexbot_ts_setting_project_root',
		...
		create_resp := codexbot_ts_dispatch_feishu_message(mut executor, mut app,
			'codexbot_ts_create_project', 'trace_codexbot_ts_create_project',
			'chat_codexbot_ts_project_registry', 'om_codexbot_ts_create_project', '/create alpha') or {
			panic(err)
		}
		...
		import_resp := codexbot_ts_dispatch_feishu_message(mut executor, mut app,
			'codexbot_ts_import_project', 'trace_codexbot_ts_import_project',
			'chat_codexbot_ts_project_registry', 'om_codexbot_ts_import_project', '/import beta ' +
			import_root) or { panic(err) }
		...
		switch_resp := executor.dispatch_websocket_upstream(mut app, transport.WorkerWebSocketUpstreamDispatchRequest{
			...
			target:      'chat_codexbot_ts_project_registry'
			target_type: 'chat_id'
			payload:     codexbot_ts_feishu_payload('/project alpha',
				'chat_codexbot_ts_project_registry', 'om_codexbot_ts_switch_to_alpha')
		}) or { panic(err) }
		...
		bind_resp := executor.dispatch_websocket_upstream(mut app, transport.WorkerWebSocketUpstreamDispatchRequest{
			...
			target:      'chat_codexbot_ts_project_registry'
			target_type: 'chat_id'
			payload:     codexbot_ts_feishu_payload('/bind beta',
				'chat_codexbot_ts_project_registry', 'om_codexbot_ts_bind_beta')
		}) or { panic(err) }
		...
		current_resp := executor.dispatch_websocket_upstream(mut app, transport.WorkerWebSocketUpstreamDispatchRequest{
			...
			target:      'chat_codexbot_ts_project_registry'
			target_type: 'chat_id'
			payload:     codexbot_ts_feishu_payload('/project',
				'chat_codexbot_ts_project_registry', 'om_codexbot_ts_project_current')
		}) or { panic(err) }
		...
		projects_resp := executor.dispatch_websocket_upstream(mut app, transport.WorkerWebSocketUpstreamDispatchRequest{
			...
			target:      'chat_codexbot_ts_project_registry'
			target_type: 'chat_id'
			payload:     codexbot_ts_feishu_payload('/projects',
				'chat_codexbot_ts_project_registry', 'om_codexbot_ts_project_list')
		}) or { panic(err) }
		assert projects_resp.handled
		assert projects_resp.commands.len == 1
...
fn test_inproc_vjsx_executor_repo_codexbot_app_ts_unbind_blocks_current_project_and_removes_non_current_binding() {
	codexbot_ts_with_temp_db('codexbot_ts_unbind.sqlite', fn (_ string) {
		...
		_ = codexbot_ts_dispatch_feishu_message(mut executor, mut app,
			'codexbot_ts_unbind_setting_project_root',
			'trace_codexbot_ts_unbind_setting_project_root', 'chat_codexbot_ts_unbind',
			'om_codexbot_ts_unbind_setting_project_root', '/setting project_root_dir ' +
			project_root) or { panic(err) }

		create_resp := codexbot_ts_dispatch_feishu_message(mut executor, mut app,
			'codexbot_ts_unbind_create_alpha', 'trace_codexbot_ts_unbind_create_alpha',
			'chat_codexbot_ts_unbind', 'om_codexbot_ts_unbind_create_alpha', '/create alpha') or {
			panic(err)
		}
		assert create_resp.handled
		assert create_resp.commands[0].text.contains('Project: `alpha`')

		bind_resp := codexbot_ts_dispatch_feishu_message(mut executor, mut app,
			'codexbot_ts_unbind_bind_beta', 'trace_codexbot_ts_unbind_bind_beta',
			'chat_codexbot_ts_unbind', 'om_codexbot_ts_unbind_bind_beta', '/bind beta') or {
			panic(err)
		}
		assert bind_resp.handled
		assert bind_resp.commands[0].text.contains('Project: `beta`')

		blocked_resp := codexbot_ts_dispatch_feishu_message(mut executor, mut app,
			'codexbot_ts_unbind_alpha_blocked', 'trace_codexbot_ts_unbind_alpha_blocked',
			'chat_codexbot_ts_unbind', 'om_codexbot_ts_unbind_alpha_blocked', '/unbind alpha') or {
			panic(err)
		}
		...
		unbind_resp := codexbot_ts_dispatch_feishu_message(mut executor, mut app,
			'codexbot_ts_unbind_beta', 'trace_codexbot_ts_unbind_beta', 'chat_codexbot_ts_unbind',
			'om_codexbot_ts_unbind_beta', '/unbind beta') or { panic(err) }
		...
		projects_resp := codexbot_ts_dispatch_feishu_message(mut executor, mut app,
			'codexbot_ts_unbind_projects', 'trace_codexbot_ts_unbind_projects',
			'chat_codexbot_ts_unbind', 'om_codexbot_ts_unbind_projects', '/projects') or {
			panic(err)
		}
		assert projects_resp.handled
		assert projects_resp.commands.len == 1
	...
	codexbot_ts_with_temp_db('codexbot_ts_legacy_project_bindings.sqlite', fn (_ string) {
		...
		_ = executor.dispatch_websocket_upstream(mut app, transport.WorkerWebSocketUpstreamDispatchRequest{
			...
			payload:     codexbot_ts_feishu_payload('/setting project_root_dir ' + project_root,
				'chat_codexbot_ts_legacy_projects',
				'om_codexbot_ts_legacy_bindings_setting_project_root')
		}) or { panic(err) }
		...
		create_resp := executor.dispatch_websocket_upstream(mut app, transport.WorkerWebSocketUpstreamDispatchRequest{
			...
			target:      'chat_codexbot_ts_legacy_projects'
			target_type: 'chat_id'
			payload:     codexbot_ts_feishu_payload('/create alpha',
				'chat_codexbot_ts_legacy_projects', 'om_codexbot_ts_legacy_bindings_create_alpha')
		}) or { panic(err) }
		assert create_resp.handled
		assert create_resp.commands.len == 1
		assert create_resp.commands[0].text.contains('Project: `alpha`')
		...
		projects_resp := executor.dispatch_websocket_upstream(mut app, transport.WorkerWebSocketUpstreamDispatchRequest{
			...
			target_type: 'chat_id'
			payload:     codexbot_ts_feishu_payload('/projects',
				'chat_codexbot_ts_legacy_projects', 'om_codexbot_ts_legacy_bindings_projects')
		}) or { panic(err) }
		assert projects_resp.handled
		assert projects_resp.commands.len == 1
		assert projects_resp.commands[0].text.contains('**Projects**')
...
fn test_inproc_vjsx_executor_repo_codexbot_app_ts_create_and_bind_failure_paths() {
	codexbot_ts_with_temp_db('codexbot_ts_create_bind_failures.sqlite', fn (_ string) {
		...
		create_alpha := codexbot_ts_dispatch_feishu_message(mut executor, mut app,
			...
			'om_codexbot_ts_create_bind_failures_create_alpha', '/create alpha') or { panic(err) }
		assert create_alpha.handled
		assert create_alpha.commands[0].text.contains('**Project Created**')

		create_alpha_again := codexbot_ts_dispatch_feishu_message(mut executor, mut app,
			'codexbot_ts_create_bind_failures_create_alpha_again',
			'trace_codexbot_ts_create_bind_failures_create_alpha_again',
			'chat_codexbot_ts_create_bind_failures',
			'om_codexbot_ts_create_bind_failures_create_alpha_again', '/create alpha') or {
			panic(err)
		}
		...
		create_gamma := codexbot_ts_dispatch_feishu_message(mut executor, mut app,
			'codexbot_ts_create_bind_failures_create_gamma',
			'trace_codexbot_ts_create_bind_failures_create_gamma',
			'chat_codexbot_ts_create_bind_failures',
			'om_codexbot_ts_create_bind_failures_create_gamma', '/create gamma') or { panic(err) }
		...
		bind_beta := codexbot_ts_dispatch_feishu_message(mut executor, mut app,
			...
			'chat_codexbot_ts_create_bind_failures',
			'om_codexbot_ts_create_bind_failures_bind_beta', '/bind beta ' + explicit_bind_root) or {
			panic(err)
		}
		assert bind_beta.handled
		assert bind_beta.commands.len == 1
		...
		bind_beta_again := codexbot_ts_dispatch_feishu_message(mut executor, mut app,
			'codexbot_ts_create_bind_failures_bind_beta_again',
			'trace_codexbot_ts_create_bind_failures_bind_beta_again',
			'chat_codexbot_ts_create_bind_failures',
			'om_codexbot_ts_create_bind_failures_bind_beta_again', '/bind beta ' +
			explicit_bind_root) or { panic(err) }
		...
		bind_missing := codexbot_ts_dispatch_feishu_message(mut executor, mut app,
			'codexbot_ts_create_bind_failures_bind_missing',
			'trace_codexbot_ts_create_bind_failures_bind_missing',
			'chat_codexbot_ts_create_bind_failures',
			'om_codexbot_ts_create_bind_failures_bind_missing', '/bind delta ' + missing_bind_root) or {
			panic(err)
		}
...
fn test_inproc_vjsx_executor_repo_codexbot_app_ts_settings_command_controls_project_root() {
	codexbot_ts_with_temp_db('codexbot_ts_settings.sqlite', fn (_ string) {
		...
		empty_resp := codexbot_ts_dispatch_feishu_message(mut executor, mut app,
			'codexbot_ts_settings_empty', 'trace_codexbot_ts_settings_empty',
			'chat_codexbot_ts_settings', 'om_codexbot_ts_settings_empty', '/settings') or {
			panic(err)
		}
		...
		create_without_setting := codexbot_ts_dispatch_feishu_message(mut executor, mut app,
			'codexbot_ts_settings_create_fail', 'trace_codexbot_ts_settings_create_fail',
			'chat_codexbot_ts_settings', 'om_codexbot_ts_settings_create_fail', '/create gamma') or {
			panic(err)
		}
		...
		setting_resp := codexbot_ts_dispatch_feishu_message(mut executor, mut app,
			'codexbot_ts_settings_update', 'trace_codexbot_ts_settings_update',
		...
		list_resp := codexbot_ts_dispatch_feishu_message(mut executor, mut app,
			'codexbot_ts_settings_list', 'trace_codexbot_ts_settings_list',
			'chat_codexbot_ts_settings', 'om_codexbot_ts_settings_list', '/settings') or {
			panic(err)
		}
		...
		create_resp := codexbot_ts_dispatch_feishu_message(mut executor, mut app,
			'codexbot_ts_settings_create_ok', 'trace_codexbot_ts_settings_create_ok',
			'chat_codexbot_ts_settings', 'om_codexbot_ts_settings_create_ok', '/create gamma') or {
			panic(err)
		}
		assert create_resp.handled
		assert create_resp.commands.len == 1
...
fn test_inproc_vjsx_executor_repo_codexbot_app_ts_instances_command_normalizes_legacy_default_alias() {
	codexbot_ts_with_temp_db('codexbot_ts_instances_legacy_alias.sqlite', fn (_ string) {
		...
		warm_resp := executor.dispatch_websocket_upstream(mut app, transport.WorkerWebSocketUpstreamDispatchRequest{
			...
			target_type: 'chat_id'
			payload:     codexbot_ts_feishu_payload('/help',
				'chat_codexbot_ts_instances_legacy_alias',
		...
		resp := executor.dispatch_websocket_upstream(mut app, transport.WorkerWebSocketUpstreamDispatchRequest{
			...
			target:      'chat_codexbot_ts_instances_legacy_alias'
			target_type: 'chat_id'
			payload:     codexbot_ts_feishu_payload('/instances',
				'chat_codexbot_ts_instances_legacy_alias', 'om_codexbot_ts_instances_legacy_alias')
		}) or { panic(err) }
		assert resp.handled
