...
fn test_runtime_snapshot_summarizes_relay_state_and_trace_ids() {
	...
	channels.enqueue('chan_1', WireFrame{
		version:       wire_version
		kind:          .data
		id:            'relay-response:frm_returned'
		trace_id:      'trace_1'
		channel_id:    'chan_1'
	...
	sessions.buffer_pending('session_1', 'link_1', new_frame(.data, 'frm_2', 'trace_2'), 4) or {
		panic(err)
	}
	...
	assert snapshot.pending_frames == 3
	assert snapshot.returned_frames == 1
	...
	assert snapshot.channels[0].buffered_len == 2
	assert snapshot.channels[0].returned_len == 1
