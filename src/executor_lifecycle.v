...
pub fn (l PhpWorkerExecutorLifecycle) start(mut app App) {
	_ = l
	if !app.worker.worker_backend.autostart {
		return
	}
	app.worker.worker_backend.managed_workers = transport.start_worker_pool(app.worker.worker_backend.cmd, app.worker.worker_backend.env,
		app.worker.worker_backend.sockets, app.worker.worker_backend.workdir)
	if app.worker.worker_backend.managed_workers.len == 0 && app.worker.worker_backend.sockets.len > 0 {
		...
		app.emit('worker.pool.empty', {
			...
			'worker_cmd':       app.worker.worker_backend.cmd
		})
	}
...
pub fn (l PhpWorkerExecutorLifecycle) stop(mut app App) {
	_ = l
	transport.stop_worker_pool(mut app.worker.worker_backend.managed_workers)
}
