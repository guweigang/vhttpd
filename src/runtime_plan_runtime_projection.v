...
struct RuntimePlanRuntimeProjection {
	...
	providers       provider.ProviderRuntimeSettings
}

fn RuntimePlanRuntimeProjection.from_plan(plan runtime_plan.RuntimePlan, listener_id string, routes []RuntimeRouteRule, assets_root string, worker_root string, primary_env map[string]string, additional_workers map[string]&worker.WorkerState, fallback_providers provider.ProviderRuntimeSettings) RuntimePlanRuntimeProjection {
	return RuntimePlanRuntimeProjection{
		...
		providers:       provider_runtime_settings_from_plan(plan, listener_id, fallback_providers)
	}
}
...
fn (mut app App) apply_runtime_plan_runtime_projection(plan runtime_plan.RuntimePlan, projection RuntimePlanRuntimeProjection) {
	...
	app.providers.apply_provider_runtime_settings(projection.providers)
}
