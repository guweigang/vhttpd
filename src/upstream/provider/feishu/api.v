...
// resolve_params resolves and validates send message parameters.
...
pub fn (req SendMessageRequest) resolve_params() !(string, string, string, string) {
	...
	content :=
		SendMessageRequest.build_content(msg_type, req.content, req.text, req.content_fields)!
	return receive_id_type, req.receive_id.trim_space(), msg_type, content
}

// api_url builds the Feishu send message API URL.
...
// api_payload builds the JSON payload for sending a Feishu message.
...
// resolve_params resolves and validates update message parameters.
...
// api_request builds the URL, HTTP method and payload for updating a Feishu message.
...
// resolve_params resolves and validates upload image parameters.
pub fn (req UploadImageRequest) resolve_params() !(string, string, string) {
	if req.data_base64 == '' {
...
// multipart_config builds the multipart form config for image upload.
...
// request_body builds the request body for tenant access token.
...
// parse parses the tenant access token response.
