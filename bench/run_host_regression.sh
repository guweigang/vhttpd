...
VSLIM_ROOT="${VSLIM_ROOT:-$(cd "${ROOT_DIR}/../vphpx/vslim" && pwd)}"
BENCH_DIR="$ROOT_DIR/bench"
HTTPD_BIN="$ROOT_DIR/vhttpd"
WORKER_PHP="$ROOT_DIR/php/package/bin/php-worker"
...
worker_cmd="VHTTPD_APP=$(q "$app_bootstrap") php -d extension=$(q "$VSLIM_SO") $(q "$WORKER_PHP")"
...
if pgrep -f "php-worker --socket ${worker_socket_prefix}_" >/dev/null 2>&1; then
  echo "worker leak detected for prefix ${worker_socket_prefix}_"
  pgrep -af "php-worker --socket ${worker_socket_prefix}_" || true
  exit 1
fi
