...
- `php-worker` (`vhttpd/php/package/bin/php-worker`)
...
- `VHTTPD_APP` is the fixed worker bootstrap environment variable name
- `VHTTPD_APP` is runtime-scoped and framework-agnostic
- renaming `VHTTPD_APP` is a breaking change and is not allowed within `v1`
...
- isolate worker bootstrap/config by setting the fixed `VHTTPD_APP` variable

Rollout guidance:

- treat stream mode as opt-in app behavior (return `VPhp\\VHttpd\\PhpWorker\\StreamResponse`)
