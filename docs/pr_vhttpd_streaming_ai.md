...
```bash
php -l /Users/guweigang/Source/vhttpd/examples/ai-stream-app.php
cd /Users/guweigang/Source/vphpx/vslim
TEST_PHP_EXECUTABLE="$(which php)" php ../vphpx/vphptest/run-tests.php -q --show-all -d extension=./vslim.so tests/test_ai_stream_demo_app.phpt
```
...
```bash
cd /Users/guweigang/Source/vphpx/vslim
TEST_PHP_EXECUTABLE="$(which php)" php ../vphpx/vphptest/run-tests.php -q --show-all -d extension=./vslim.so tests/test_httpd_worker_streaming.phpt
```
