...
- `php/package/bin/php-worker` 负责 app bootstrap、PSR 适配与响应归一
...
- 当前可选桥接：`VPhp\\VHttpd\\Psr7Adapter` 将 envelope 转为 PSR-7 request 对象
- 已内置工厂检测：
...
- `VPhp\\VSlim\\Psr7Adapter` 作为“示例桥接”，不是强制路径
...
- 回归命令：`make -C /Users/guweigang/Source/vhttpd psr-matrix`
- 若 `composer` 不在默认 PATH，可指定：`COMPOSER_BIN=/abs/path/to/composer make -C /Users/guweigang/Source/vhttpd psr-matrix`
