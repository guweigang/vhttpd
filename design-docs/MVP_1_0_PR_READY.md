...
Expected:
- binary exists at `/Users/guweigang/Source/vhttpd/vhttpd`
- `./vhttpd --help` prints usage and exits

## 2. Config startup (TOML)

```bash
/Users/guweigang/Source/vhttpd/vhttpd \
  --config /Users/guweigang/Source/vhttpd/examples/config/laravel.toml
```
...
```bash
pgrep -fal 'php-worker|vhttpd'
```

Expected:
- no orphan `php/package/bin/php-worker` process remains
