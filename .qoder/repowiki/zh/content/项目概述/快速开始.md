...
## 更新摘要
**所做更改**
- 更新了快速开始部分，添加了新的终端风格代码演示
- 增强了 PHP 和 TypeScript 应用的启动命令示例
- 改进了配置说明和启动参数展示
- 优化了用户体验，提供更直观的入门指导
...
- 使用 TOML 配置（推荐）
  ...
  - 示例配置：
    ...
    - VJSX 示例：config/vhttpd.vjsx.example.toml
- **新增** 终端风格快速启动命令
  - 一行启动 PHP 应用：`./vhttpd --config config/vhttpd.example.toml`
  - 一行启动 TypeScript 应用：`./vhttpd --executor vjsx --vjsx-entry app.mts`
- 常用启动参数（节选）
  ...
  - --executor, --vjsx-entry, --vjsx-build-root, --vjsx-runtime-profile, --vjsx-thread-count：VJSX 专用参数
  - 更多参数见帮助输出

**更新** 新增了终端风格的快速启动命令示例，让用户能够更直观地了解如何快速启动不同类型的应用。
...
- **更新** 快速启动命令
  - 终端风格：`./vhttpd --config config/vhttpd.example.toml`
  - 传统方式：`./vhttpd --config config/vhttpd.example.toml`
- 验证
...
- **更新** 快速启动命令
  - 终端风格：`./vhttpd --executor vjsx --vjsx-entry app.mts`
  - 完整配置：`./vhttpd --config config/vhttpd.vjsx.example.toml`
- 验证
  - 访问根路径或示例路由，查看返回的 JSON（包含 method/path/name 等字段）
- 关键实现位置
  - VJSX 处理器位于 examples/vjsx/hello-handler.mts

**更新** 新增了终端风格的启动命令，使 TypeScript 应用的启动更加简洁直观。
...
- VJSX 模式启动失败
  ...
  - 检查 --executor vjsx 参数是否正确指定
- 管理面不可用
  - 检查 --admin-host/--admin-port/--admin-token 配置
  - 确保防火墙允许访问管理面端口

**更新** 增加了 VJSX 模式启动失败的排查项，包括 --executor vjsx 参数的检查。
...
- 常用命令速查
  ...
  - 帮助：./vhttpd --help
  - **新增** 终端快速启动：./vhttpd --executor vjsx --vjsx-entry app.mts
- 参考路径
