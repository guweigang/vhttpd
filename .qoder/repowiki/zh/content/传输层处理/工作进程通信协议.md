...
Main->>Worker : "写入帧(JSON)"
Worker-->>Main : "写回帧(JSON)"
...
图表来源
- [worker_framing.v:10-46](file://src/transport/worker_framing.v#L10-L46)
...
图表来源
- [worker_framing.v:124-153](file://src/transport/worker_framing.v#L124-L153)
...
sequenceDiagram
participant Main as "主进程"
participant Conn as "Unix 连接"
participant Worker as "工作进程"
Main->>Conn : "写入 StreamDispatchRequest 帧"
Worker-->>Conn : "写入 StreamDispatchResponse 帧"
...
## 结论
vhttpd 工作进程通信协议以“4 字节长度前缀 + JSON 帧”的简洁设计实现了高可靠与高扩展性。通过清晰的消息类型划分、严格的错误分类与退避重连策略，系统在复杂场景下仍能保持稳定与可控。遵循本文档的扩展与调试建议，可进一步提升协议的演进能力与运行效率。

## 附录
- 关键实现位置参考
  - 帧读写与 HTTP 辅助：[worker_framing.v:10-46](file://src/transport/worker_framing.v#L10-L46)
  - 请求编码：[worker_framing.v:124-153](file://src/transport/worker_framing.v#L124-L153)
