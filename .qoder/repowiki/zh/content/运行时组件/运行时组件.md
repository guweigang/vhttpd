...
participant RT as "运行时模块(mcp/websocket/stream)"
...
## 依赖关系分析
- 组件耦合
  - 运行时模块（MCP/WS/Stream/Admin）均依赖 server 作为统一入口
...
- 关键日志与审计
  ...
  - 运行时在关键路径（会话绑定/解绑、上游连接状态变更）产生事件
- 建议操作
  - 使用管理快照接口快速评估当前运行时状态
...
## 结论
vhttpd 的运行时组件以模块化设计实现了从请求入口到流式输出的完整链路。MCP 与 WebSocket 提供了强大的实时通信能力，配合上游运行时与工作池传输层，满足复杂场景下的高并发与可靠性需求。管理运行时提供了可观测性与运维能力，便于在生产环境中进行监控与排障。
